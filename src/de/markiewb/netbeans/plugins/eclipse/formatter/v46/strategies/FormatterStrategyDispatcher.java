...
import de.markiewb.netbeans.plugins.eclipse.formatter.v46.Utilities;
import de.markiewb.netbeans.plugins.eclipse.formatter.v46.strategies.eclipse.EclipseFormatter;
import de.markiewb.netbeans.plugins.eclipse.formatter.v46.strategies.eclipse.EclipseFormatter.CannotLoadConfigurationException;
import de.markiewb.netbeans.plugins.eclipse.formatter.v46.strategies.eclipse.EclipseFormatter.ProfileNotFoundException;
...
import static de.markiewb.netbeans.plugins.eclipse.formatter.v46.options.Preferences.getActivePreferences;
import static de.markiewb.netbeans.plugins.eclipse.formatter.v46.options.Preferences.getLineFeed;
import de.markiewb.netbeans.plugins.eclipse.formatter.v46.strategies.eclipse.EclipseFormatterStrategy;
import de.markiewb.netbeans.plugins.eclipse.formatter.v46.strategies.netbeans.NetBeansFormatterStrategy;
import java.io.File;
...
public class FormatterStrategyDispatcher {
    ...
    public void format(ParameterObject po) {
        ...
        if (!hasGuardedSections && isJava && isEclipseFormatterEnabled) {
            ...
            if (showNotifications) {
                NotificationDisplayer.getDefault().notify("Format using Eclipse formatter 4.6", Utilities.iconEclipse, msg, null);
            }
            StatusDisplayer.getDefault().setStatusText("Format using Eclipse formatter 4.6: " + msg);
    ...
    public String getNotificationMessageForEclipseFormatterConfigurationFileType(String formatterFile, String formatterProfile) {
        String msg = "";
        if (de.markiewb.netbeans.plugins.eclipse.formatter.v46.options.Preferences.isWorkspaceMechanicFile(formatterFile)) {
            //Workspace mechanic file
            msg = String.format("Using %s", formatterFile);
        } else if (de.markiewb.netbeans.plugins.eclipse.formatter.v46.options.Preferences.isXMLConfigurationFile(formatterFile)) {
            //XML file
            msg = String.format("Using profile '%s' from %s", formatterProfile, formatterFile);
        } else if (de.markiewb.netbeans.plugins.eclipse.formatter.v46.options.Preferences.isProjectSetting(formatterFile)) {
